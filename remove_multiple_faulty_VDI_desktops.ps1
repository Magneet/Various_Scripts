...
try {
	$services1.machine.Machine_DeleteMachines($desktops.id, $spec)
	write-host "Desktops in $targetstate state are marked for deletion" -ForegroundColor Green
}
catch {
	write-host "Error deleting desktops in $targetstate state" -ForegroundColor Red
}
